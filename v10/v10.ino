...
void setup() {
  ...
  xTaskCreatePinnedToCore(Task1code, "Task1", 5000, NULL, 0, &Task1, 0);
  delay(500);
      ...
      while (menu == -1) {
        ...
        if (digitalRead(sw) == 0 && menu == -1 && count == 1) {
          while (digitalRead(sw) == 0 && menu == -1 && count == 1)
            ;
          if (start_stop == 0 && temp > set_temp - 5 && temp < set_temp + 5) {
            stby_en = 1;
            ready_menu[1] = "Stop            ";
            count_o = -1;
            k_memu--;
...
void Pid_Temp() {
  if (temp >= Chamber_temp && set_temp > 0 && temp <= Max_temp) {
    ...
    if (currentTime - lastTime >= sampleTime) {
      ...
      PID_Val = (Kp * error) + Ki + (Kd * derivative);
